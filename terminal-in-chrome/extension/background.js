chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'TERMINAL_ACTIVITY' && sender.tab) {
    if (message.status === 'processing') {
      chrome.action.setBadgeText({ text: '...', tabId: sender.tab.id });
      chrome.action.setBadgeBackgroundColor({ color: '#03A9F4', tabId: sender.tab.id });
      if (chrome.action.setBadgeTextColor) chrome.action.setBadgeTextColor({ color: '#FFFFFF', tabId: sender.tab.id });
    } else if (message.status === 'waiting') {
      chrome.action.setBadgeText({ text: '!', tabId: sender.tab.id });
      chrome.action.setBadgeBackgroundColor({ color: '#F44336', tabId: sender.tab.id });
      if (chrome.action.setBadgeTextColor) chrome.action.setBadgeTextColor({ color: '#FFFFFF', tabId: sender.tab.id });
    } else if (message.status === 'clear') {
      chrome.action.setBadgeText({ text: '', tabId: sender.tab.id });
    }
  }
});